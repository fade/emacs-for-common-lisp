...
#+BEGIN_SRC emacs-lisp
  (use-package sly
    ...
    (progn
      ...
      (require 'sly-tramp)
      ;; Default dynamic translator: uses the lisp's machine-instance as the TRAMP host.
      ;; The username is derived from (user-login-name) but can be overridden by the USER environment variable.
      (let ((lisp-user (or (getenv "USER") (user-login-name))))
        (setq sly-filename-translations
              (list (list "^.*$"
                          (lambda (emacs-filename)
                            (tramp-file-name-localname (tramp-dissect-file-name emacs-filename)))
                          `(lambda (lisp-filename)
                             (let ((conn (sly-current-connection)))
                               (sly-make-tramp-file-name
                                ,lisp-user
                                (sly-machine-instance conn)
                                lisp-filename))))))

        ;; Specific override for the synthia cluster to ensure we always use the head node (hn1)
        ;; for the TRAMP connection, even if the machine-instance is just "synthia".
