...
#+begin_src emacs-lisp
  ;;;  config.el ---  DeepSky emacs config file  -*- lexical-binding: t; -*-
...
[[https://github.com/karthink/popper][Popper]] solves the problem where you summon a repl or similar to do a
calculation, which then disturbs the feng shui of your carefully laid
out windows. Popper puts the windows in a popup overlay which can be
dismissed into the background after you are done with it.
#+begin_src emacs-lisp :tangle no
  (use-package popper
    :straight t
...
Calling use-package with 'emacs as the package target alows top level
init overrides of things done in early-init.el or in init.el

#+begin_src emacs-lisp
  (use-package emacs
    :init
    ;; global emacs things here, often to override things that happened
    ;; in init.el enable indentation+completion using the TAB key.
...
#+begin_src emacs-lisp
  (use-package jinx
    ...
    :config
    ;; Use Canadian, British, and US English in order of preference.
    (setq jinx-languages "en_CA en_GB en_US")
    ;; Only enable jinx in text-mode (natural language files)
    ;; and exclude it from programming modes and dashboard.
    (setq global-jinx-modes '(text-mode))
    ;; Ensure org-mode code blocks and other meta-data are excluded.
    ;; Jinx usually handles this by default, but we'll be explicit.
    (add-to-list 'jinx-exclude-faces '(org-mode org-block org-block-begin-line org-block-end-line
                                               org-code org-verbatim org-meta-line)))
#+end_src

* Personal keymaps
Personal keymaps are located under the =C-q= prefix. The =C-q v= prefix
map provides a unified interface for AI assistants and agents,
including both gptel (for inline assistant tasks) and agent-shell (for
autonomous project-level tasks).

| Prefix | Key | Command            | Description                                  |
|--------+-----+--------------------+----------------------------------------------|
| C-q v  | g   | gptel-send         | Send the active region or buffer to gptel    |
| C-q v  | m   | gptel-menu         | Open the gptel configuration transient menu  |
| C-q v  | r   | gptel-rewrite      | Refactor or rewrite the active region        |
| C-q v  | c   | agent-shell claude | Start a Claude agent shell                   |
| C-q v  | s   | agent-shell        | Start or switch to an autonomous agent shell |
| C-q v  | d   | agent-shell defun  | Send the current Lisp defun to the agent     |
|--------+-----+--------------------+----------------------------------------------|
...
#+begin_src emacs-lisp
  ...
  (defvar-keymap prefix-buffer-map-0
    :doc "Prefix map for C-q for CrafterBin"
    "f" #'crafterbin-upload-file
    "t" #'crafterbin-upload)

  (defvar-keymap prefix-buffer-map-a
    :doc "Prefix map for C-q for ace-window"
    "o" #'ace-window
    "s" #'ace-swap-window)
  ...
  (defvar-keymap prefix-buffer-map-k
    :doc "Prefix map for C-q for spelling (Check)"
    ;; "o" #'osx-dictionary-search-word-at-point
    "l" #'jinx-languages
    "c" #'jinx-correct
    "s" #'dictionary-search)

  (defvar-keymap prefix-buffer-map-sly
    :doc "Prefix map for C-q for SLY"
    "s" #'sly
    "c" #'sly-connect
    "r" #'fade/sly-connect-remote
    "d" #'sly-disconnect)
  ...
  (defvar-keymap prefix-buffer-map-v
    :doc "Prefix map for C-q for AI (gptel/agent-shell)"
    "g" #'gptel-send
    "m" #'gptel-menu
    "r" #'gptel-rewrite
    "c" (lambda () (interactive) (let ((agent-shell-default-agent "claude")) (agent-shell)))
    "o" (lambda () (interactive) (let ((agent-shell-default-agent "opencode")) (agent-shell)))
    "s" #'agent-shell
    "d" (lambda () (interactive) (save-excursion (mark-defun) (agent-shell))))
...
#+BEGIN_SRC emacs-lisp
  ...
  (progn
    ...
    ;; Enable mixed fonts so modus-themes-fixed-pitch inherits from
    ;; fixed-pitch. Without this, org-block, org-code, org-table, etc.
    ;; all render in variable-pitch when variable-pitch-mode is on.
    (setq modus-themes-mixed-fonts t)
    ;; (load-theme 'deepsky-modus-fade t)
    (require 'deepsky-moonunit-theme)
    (load-theme 'deepsky-moonunit t nil)
    (message "Configuring mode-line appearance...")
    (set-face-attribute `mode-line nil
                        :box nil)
    (blink-cursor-mode 1)
    (setf blink-cursor-blinks -1)
...
the box. [[https://emacs.stackexchange.com/a/28604][Others have run into the same issue, also]]. The solution is to
...
* Dotfiles
I had hopes for this, but it doesn't work.
#+begin_src emacs-lisp :tangle no
  (use-package chezmoi
    :straight t)
#+end_src
...
#+BEGIN_SRC emacs-lisp
  ...
  (use-package treesit-parser-manager
    ...
    (treesit-parser-manager-grammars
     '(("https://github.com/tree-sitter-grammars/tree-sitter-rust"
        ("tree-sitter-rust"))

       ("https://github.com/ikatyang/tree-sitter-toml"
        ("tree-sitter-toml"))

       ("https://github.com/tree-sitter-grammars/tree-sitter-typescript"
        ("tree-sitter-typescript/tsx" "tree-sitter-typescript/typescript"))

       ("https://github.com/tree-sitter-grammars/tree-sitter-javascript"
        ("tree-sitter-javascript"))

       ("https://github.com/tree-sitter-grammars/tree-sitter-css"
        ("tree-sitter-css"))

       ("https://github.com/serenadeai/tree-sitter-scss"
        ("tree-sitter-scss"))

       ("https://github.com/tree-sitter-grammars/tree-sitter-json"
        ("tree-sitter-json"))

       ("https://github.com/tree-sitter-grammars/tree-sitter-go"
        ("tree-sitter-go"))

       ("https://github.com/tree-sitter-grammars/tree-sitter-cpp"
        ("tree-sitter-cpp"))

       ;; ("https://github.com/tree-sitter-grammars/tree-sitter-c"
       ;;  ("tree-sitter-c"))
...
* lsp-mode benefits most programming modes, drop it in before they are initialized.
#+BEGIN_SRC emacs-lisp
...
* Agent Shell
[[https://github.com/xenodium/agent-shell][Agent Shell]] provides a native Emacs interface for interacting with LLM agents using the Agent Client Protocol (ACP). It supports Gemini CLI, Claude Code, and other agents.

#+BEGIN_SRC emacs-lisp
  (use-package agent-shell
    :straight (agent-shell :type git :host github :repo "xenodium/agent-shell")
    :bind (:map prefix-buffer-map-v
                ("c" . (lambda () (interactive) (let ((agent-shell-default-agent "claude")) (agent-shell))))
                ("o" . (lambda () (interactive) (let ((agent-shell-default-agent "opencode")) (agent-shell))))
                ("s" . agent-shell)
                ("d" . agent-shell-send-defun))
    :hook (agent-shell-mode . agent-shell-completion-mode)
    :init
    ;; Patch shell-maker--curl-version-supported globally in :init to ensure it's
    ;; active before any part of agent-shell or shell-maker tries to call it.
    (with-eval-after-load 'shell-maker
      (defun shell-maker--curl-version-supported ()
        ...
        (let* ((curl (if (boundp 'shell-maker-curl-executable) shell-maker-curl-executable "curl"))
               (curl-version-string (shell-command-to-string (concat curl " --version "))))
          (if (string-match "\\([0-9]+\\)\\.\\([0-9]+\\)\\.\\([0-9]+\\)" curl-version-string)
              (let ((major (string-to-number (match-string 1 curl-version-string)))
                    (minor (string-to-number (match-string 2 curl-version-string))))
                (or (> major 7)
                    (and (= major 7) (>= minor 76))))
            nil))))
    :config
    ;; Default to Gemini CLI.
    (setq agent-shell-default-agent "gemini")
    ;; Ensure gemini-cli is called with the ACP flag.
    (setq agent-shell-google-gemini-acp-command '("gemini" "--acp"))
    ;; Ensure claude-code is called with the ACP flag.
    (setq agent-shell-anthropic-claude-acp-command '("claude" "--acp"))
    ;; Ensure opencode is called with the ACP flag.
    (setq agent-shell-opencode-acp-command '("opencode" "acp"))

    (defun agent-shell-send-defun ()
      "Send the current Lisp defun to the agent shell."
      (interactive)
      (save-excursion
        (mark-defun)
        (agent-shell (buffer-substring-no-properties (region-beginning) (region-end)))))

    ;; Integrate with Sly for Common Lisp workflows.
    (with-eval-after-load 'sly
      (define-key sly-mode-map (kbd "C-q v c")
                  (lambda () (interactive) (let ((agent-shell-default-agent "claude")) (agent-shell))))
      (define-key sly-mode-map (kbd "C-q v o")
                  (lambda () (interactive) (let ((agent-shell-default-agent "opencode")) (agent-shell))))
      (define-key sly-mode-map (kbd "C-q v d") #'agent-shell-send-defun)
      (define-key sly-mode-map (kbd "C-q v s") #'agent-shell)))
#+end_src
...
(use-package recentf
  ...
  ;; Disable auto-cleanup, especially for TRAMP files which causes hangs/errors.
  (setq recentf-auto-cleanup 'never)
  (add-to-list 'recentf-exclude "^/\\(?:ssh\\|sshx\\|scp\\|pscp\\|sftp\\|adb\\|docker\\|sudo\\|flatpak\\):"))
#+END_SRC

* CrafterBin
post regions/files/iota of emacs buffers to CrafterBin for linking in remote places like IRC.
#+BEGIN_SRC emacs-lisp
  (use-package crafterbin
    :straight (crafterbin :type git :host github :repo "glenneth1/crafterbin.el"))
#+end_src
...
#+BEGIN_SRC emacs-lisp
  (use-package tramp
    ...
    (add-to-list 'tramp-connection-properties
                 (list (format "/\\(?:ssh\\|sshx\\|scp\\):.*%s:" (regexp-quote (user-login-name)))
                       "remote-shell" "/bin/sh"))
    ;; Generalize remote shell to all ssh/scp connections for stability.
    (add-to-list 'tramp-connection-properties
                 (list "/\\(?:ssh\\|sshx\\|scp\\):"
                       "remote-shell" "/bin/sh"))
...
#+BEGIN_SRC emacs-lisp 
  (use-package beacon
    ...
    (beacon-dont-blink-major-modes '(agent-shell-mode agent-shell-diff-mode)))
...
#+BEGIN_SRC emacs-lisp 
  (use-package gptel
    ...
    (gptel-make-anthropic "Claude"
                          :stream t
                          :key (lambda () (auth-source-pick-first-password :host "api.anthropic.com" :user "apikey")))
...
#+BEGIN_SRC emacs-lisp
  ...
  (use-package git-ps1-mode
    ...
    :after magit
    :config
    (add-hook 'prog-mode-hook 'git-ps1-mode)
...
** Common Lisp
*** SLY
 Sly is superior to Slime.
    
#+BEGIN_SRC emacs-lisp
  (use-package sly
    :load-path ("~/SourceCode/lisp/sly" "~/SourceCode/lisp/sly/contrib")
    :straight t
    :commands sly
    :bind
    ...
    (progn
      (require 'cl-lib)
      (defvar sly-elide-output nil)
      (defvar corepath (expand-file-name "sbcl.core" (getenv "HOME")))
      ;; (load (expand-file-name ".roswell/helper.el" (getenv "HOME")))
      (if (eql system-type 'gnu/linux)
          (setf sly-lisp-implementations
                '((sbcl ("sbcl" "--dynamic-space-size" "4GB"))
                  ...
                  (decl ("/usr/bin/ecl")))
                sly-contribs '(sly-fancy sly-tramp sly-quicklisp sly-stepper)
        ...
        (setf sly-lisp-implementations
              ...
              sly-default-lisp 'sbcl
              common-lisp-hyperspec-root "file:///home/fade/SourceCode/lisp/HyperSpec/"))
      ;; filename translations between emacs and the remote lisp.
      (require 'sly-tramp)

      (defun fade/sly-connect-remote (host port)
        "Connect to a remote Slynk server via SSH tunnel.
HOST is the remote hostname (for TRAMP), PORT is the local tunnel port."
        (interactive (list (read-string "Remote Host (for TRAMP): ")
                           (read-number "Local Port: " 4005)))
        (let ((lisp-user (or (getenv "USER") (user-login-name))))
          ;; Ensure we have a translator for this machine-instance
          (add-to-list 'sly-filename-translations
                       (sly-create-filename-translator
                        :machine-instance host
                        :remote-host host
                        :username lisp-user)))
        (sly-connect "localhost" port))

      ;; Specific overrides for known clusters
      (let ((lisp-user (or (getenv "USER") (user-login-name))))
        (dolist (host '("synthia" "synthia-hn1" "synthia-hn1.res.nrc.gc.ca"))
          (add-to-list 'sly-filename-translations
                       (sly-create-filename-translator
                        :machine-instance host
                        :remote-host "synthia-hn1"
                        :username lisp-user))))

      ;; Connection-local variables for synthia to use Roswell.
      ...
      (dolist (host '("synthia" "synthia-hn1" "synthia-hn1.res.nrc.gc.ca"))
        (connection-local-set-profiles
         `(:application tramp :machine ,host) 'synthia-lisp-profile))
      

      ;; the prompt should live at the bottom of the screen; this isn't like... good, but it's better.
      ...
      (add-to-list 'load-path (expand-file-name "~/SourceCode/lisp/log4cl/elisp"))
      (load (expand-file-name "~/SourceCode/lisp/log4cl/elisp/log4sly.el"))
  ...
  (use-package sly-asdf
    ...
    :straight (sly-asdf :host github :repo "fade/sly-asdf"
                        :branch "fix/normalize-dependency-defs")
    :after sly)
...
*** alive-lsp: Common Lisp Language Server

[[https://github.com/nobody-famous/alive-lsp][alive-lsp]] is a Common Lisp implementation of the Language Server
Protocol that communicates over TCP. It runs inside an SBCL process (started
from =~/.sbclrc= at SBCL launch time) and provides the server side of the LSP
channel that =lsp-mode= connects to on port 8006.

The governing principle here is *SLY-first, LSP-fallback*. SLY owns completion
(via its own CAPF and corfu integration) and the REPL/eval/debug workflow.
=lsp-mode= provides the three features SLY does not cover well for multi-file
projects: cross-file jump-to-definition, hover documentation on demand, and
inline diagnostics via the flycheck backend. LSP completion is disabled
buffer-locally so =company= or =corfu= candidates come from SLY, not the language
server.

Integration is handled by =cl-lsp-integration.el= in =fade/= (the local
customisation directory already on =load-path=). It is loaded after =lsp-mode=
has initialised, with a =locate-library= guard so Emacs continues to work
normally even if the file is temporarily absent.

#+BEGIN_SRC emacs-lisp
  ;; Common Lisp LSP integration (LSP-01 through LSP-06)
  ;; alive-lsp TCP server is started from ~/.sbclrc when SBCL launches.
  ;; cl-lsp-integration.el (in fade/) wires lsp-mode into lisp-mode with
  ;; SLY-first arbitration: SLY owns completion and REPL; LSP provides
  ;; cross-file navigation (M-.), hover docs on demand (C-c l d), and
  ;; inline diagnostics.  Manual workspace refresh: C-c l r.
  (with-eval-after-load 'lsp-mode
    (when (locate-library "cl-lsp-integration")
...
web-mode provides syntax support for html typed and related files.
#+begin_src emacs-lisp
  (require 'lass) ;; mode to support shinmera's common lisp LASS compiler
...
* DNS-Mode
We need to edit Bind9 zone files with distressing regularity. I need
   to just buckle down and finally fix my nameservice solution so
   there's decent tooling around it.
...
#+BEGIN_SRC emacs-lisp
  (if window-system
      (progn
        ;; Fontaine owns the fonts end-to-end: family, height, and pitch all
        ;; come from the active preset. Deliberately no default-frame-alist
        ;; font entry — a sizeless one forces the initial frame to the font's
        ;; native size and outranks the preset on that frame.
        (use-package fontaine
          ...
          ;;   (fontaine-add-preset 'custom-comments-italic
          ;;                        '(:font-lock-comment-face (:slant italic))))

          (fontaine-mode 1)

          ;; Fontaine 3.x applies presets through an always-enabled synthetic
          ;; `fontaine' theme that outranks other themes, so the fonts survive
          ;; `load-theme' on their own — no enable-theme hook is needed.
          ;; (`fontaine-apply-current-preset' was removed in 3.0.)
          )))
#+END_SRC
...
#+BEGIN_SRC emacs-lisp 
  ...
  (use-package consult-notes
    :straight (consult-notes :host codeberg :repo "mclear-tools/consult-notes"))
