...
#+BEGIN_SRC emacs-lisp
  (use-package agent-shell
    ...
    :init
    ;; Force shell-maker--curl-version-supported to return t.
    ;; The upstream version check frequently fails on major version boundaries (like curl 8.5.0).
    ;; We apply this both as an advice and as a direct definition to ensure it sticks.
    (defun my-skip-curl-check (&rest _) t)
    (advice-add 'shell-maker--curl-version-supported :override #'my-skip-curl-check)
    (with-eval-after-load 'shell-maker
      (defun shell-maker--curl-version-supported () t))
    :config
    ...
    (defun agent-shell-local ()
      "Start agent-shell on the local machine."
      (interactive)
    ...
    (with-eval-after-load 'sly
      ...
      (define-key sly-mode-map (kbd "C-q v s") #'agent-shell-local)))
#+end_src
