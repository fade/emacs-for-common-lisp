...
#+BEGIN_SRC emacs-lisp
  (use-package agent-shell
    ...
    (setq agent-shell-google-gemini-acp-command '("gemini" "--experimental-acp"))
    ;; Ensure claude-code is called with the ACP flag.
    (setq agent-shell-anthropic-claude-acp-command '("claude" "--experimental-acp"))
    ...
    (with-eval-after-load 'sly
      ...
      (define-key sly-mode-map (kbd "C-q v d") 
                  (lambda () (interactive) (save-excursion (mark-defun) (agent-shell))))
...
#+BEGIN_SRC emacs-lisp
  (use-package tramp
    ...
    (add-to-list 'tramp-connection-properties
                 (list "/\\(?:ssh\\|sshx\\|scp\\):.*fade@deepsky\\.com:"
                       "remote-shell" "/bin/sh"))
    (dolist (host '("synthia" "synthia-hn1" "synthia-hn1.res.nrc.gc.ca"))
      (add-to-list 'tramp-connection-properties
                   (list (format "/\\(?:ssh\\|sshx\\|scp\\):.*%s:" (regexp-quote host))
                         "remote-shell" "/bin/sh")))
