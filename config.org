...
#+BEGIN_SRC emacs-lisp
  (if window-system
      (progn
        ;; Family hint so the initial/new frames come up right; the fontaine
        ;; presets below own the default/fixed-pitch/variable-pitch faces —
        ;; family, height, and pitch all come from the active preset.
